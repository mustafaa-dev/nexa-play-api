import { Notification } from '@core/entities/notification.entity';
import { User } from '@core/entities/user.entity';
import { INotificationRepository } from '@core/repositories/notification.repository.interface';
import { NotificationEntity } from '@infrastructure/database/entities/notification.entity';
import { Inject, Injectable } from '@nestjs/common';
import { IQuery, IQueryHandler, QueryHandler } from '@nestjs/cqrs';
import {
  FilterOperator,
  paginate,
  PaginateConfig,
  Paginated,
  PaginateQuery,
} from 'nestjs-paginate';

export class GetNotificationsQuery implements IQuery {
  constructor(
    public readonly user: User,
    public readonly paginateQuery: PaginateQuery,
  ) {}
}

@Injectable()
@QueryHandler(GetNotificationsQuery)
export class GetNotificationsQueryHandler implements IQueryHandler<GetNotificationsQuery> {
  constructor(
    @Inject('NotificationRepository')
    private readonly notificationRepository: INotificationRepository,
  ) {}

  async execute(query: GetNotificationsQuery): Promise<Paginated<Notification>> {
    const { user, paginateQuery } = query;

    const data = await paginate(
      paginateQuery,
      this.notificationRepository.repo,
      this.getPaginationConfig(user),
    );

    return {
      ...data,
      data: data.data.map(notification => Notification.fromStore(notification).toJSON()),
    } as Paginated<Notification>;
  }

  private getPaginationConfig(user: User): PaginateConfig<NotificationEntity> {
    const config: PaginateConfig<NotificationEntity> = {
      defaultSortBy: [['createdAt', 'DESC']],
      sortableColumns: ['createdAt', 'type', 'user', 'title', 'message', 'isRead'],
      filterableColumns: {
        isRead: [FilterOperator.EQ],
      },
      relations: {
        user: true,
      },
      where: {
        user: {
          id: user.id,
        },
      },

      ignoreSelectInQueryParam: true,
    };

    return config;
  }
}
