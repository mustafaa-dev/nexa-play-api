...
import { LocalizedInvalidInputException, LocalizedThrottlingException } from '@infrastructure/i18n';
...
export class ThrottlerService implements IThrottlerService {
  ...
  async isAllowed(
    ...
    if (!identifier) {
      throw new LocalizedInvalidInputException('throttle_identifier');
    }
  ...
  async trackRequest(
    ...
    if (record.count >= throttleLimit.getLimit) {
      throw new LocalizedThrottlingException(
